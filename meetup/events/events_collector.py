#####
# One line summary.
#	This script creates a dictionary of rsvps linking events to people. 
# 
# Usage:
...
# ugly code ... i know ... this collects all the events 
event_list  	= [] 
website1  		= 'https://api.meetup.com/2/events?key=3ad7a28717926121647c7618457a2&sign=true&status=past&group_urlname=appsterdam&page=200&offset=0'
website2  		= 'https://api.meetup.com/2/events?key=3ad7a28717926121647c7618457a2&sign=true&status=past&group_urlname=appsterdam&page=200&offset=1'
new_events1  	= json.loads(urllib.urlopen(website1).read(), 'ISO-8859-1').items()[1][1]
new_events2 	= json.loads(urllib.urlopen(website2).read(), 'ISO-8859-1').items()[1][1]
event_list.extend(new_events1)
event_list.extend(new_events2)
...
for event in event_list: 
	...
	if event_id not in event_dict.keys(): 
		current_event							= {} 
		current_event['name']			= str(event['name'])
		current_event['id']				= str(event['id'])
		current_event['date'] 		= str(event['time'])
		# current_event['lat']			= event['venue']['lat']
		# current_event['lon']			= event['venue']['lon']
		# current_event['desc']			= event['description']
		website 									= 'https://api.meetup.com/rsvps?key=3ad7a28717926121647c7618457a2&sign=true&rsvp=yes&event_id=' + event_id + '&page=200'
		rsvp_members				 			= json.loads(urllib.urlopen(website).read(), 'ISO-8859-1').items()[1][1]
		if str(rsvp_members) == 'limit': 
			break 
		members = [] 
		for member in rsvp_members: 
			members.append(member['name'])
			if str(unicodedata.normalize('NFKD', member['name']).encode('ascii','ignore')) not in person_dict.keys(): 
				new_dict = {} 
				new_dict['name'] = str(unicodedata.normalize('NFKD', member['name']).encode('ascii','ignore'))
				new_dict['events'] = [current_event['id']]
				person_dict[member['name']] = new_dict
			if member['name'] in person_dict.keys(): 
				person_dict[member['name']]['events'].extend(current_event)
		new_rsvp_members = [] 
		for i in rsvp_members:
			new_dict = {'name': str(unicodedata.normalize('NFKD', i['name']).encode('ascii','ignore')), 'id': str(unicodedata.normalize('NFKD', i['id']).encode('ascii','ignore'))}
			new_rsvp_members.append(new_dict)
		current_event['members'] 	= new_rsvp_members
		event_json.append({'event_id': event_id, 'event_data':current_event})

for event_id in event_dict: 
	if len(event_dict[event_id]['members']) == 0:
		event_list.pop(event_id)

for person in person_dict: 
	new_dict = {'name': person['name'], 'events': person}
	person_json.append(new_dict)

import csv
import unicodedata
with open('member_companies.csv', 'wb') as test_file:
	file_writer = csv.writer(test_file)
	for member in member_json: 
		mem_id 		= str(member['id'])
		if 'name' in member.keys(): 
			mem_name 	= str(unicodedata.normalize('NFKD', member['name']).encode('ascii','ignore'))
		if 'bio' in member.keys(): 
			mem_bio 	= str(unicodedata.normalize('NFKD', member['bio']).encode('ascii','ignore'))
		file_writer.writerow([mem_id,mem_name, '',mem_bio])


json_string = json.dumps(event_json)
myfile = open('event_json.json', 'wb')
